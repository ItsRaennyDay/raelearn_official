...
const REC: Record<TypeKey, { path: string; desc: string; first: string }> = {
  va:          { path: "New VA Path",                    desc: "Build the core support skills every VA needs, then specialize.",         first: "Inbox & Calendar Basics" },
  freelancer:  { path: "Founder Support VA Path",        desc: "Turn freelance skills into reliable client systems.",                    first: "Beginner VA Business Toolkit" },
...
export default function SignupPage() {
  ...
  const [interests, setInterests]   = useState<string[]>(["nonprofit_ops"]);
    ...
    return (
      <div className="bg-rl-bg min-h-screen flex items-center justify-center px-7 py-20" style={{ fontFamily: "var(--font-sans)" }}>
        <div className="max-w-[480px] text-center">
          ...
          <Link href="/" className="text-[14px] font-bold text-rl-forest hover:underline">
            ← Back to home
          </Link>
        </div>
      </div>
        ...
        <div className="grid gap-[30px] items-start" style={{ gridTemplateColumns: "1.55fr 1fr" }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="bg-white border border-rl-border rounded-2xl p-[26px]">
              ...
              <p className="text-[12.5px] text-rl-dim mt-3.5 text-center">
                ...
                <Link href="/signin" className="text-rl-forest font-bold hover:underline">
                  Sign in
                </Link>
              </p>
            </div>
          ...
          <div className="sticky top-[88px]">
            <div className="bg-[#F0F5F1] border border-rl-border rounded-[18px] p-6">
              ...
              <h2 className="font-head font-extrabold text-[22px] mb-1.5 text-rl-forest">{rec.path}</h2>
              <p className="text-[13.5px] leading-[1.55] text-rl-muted mb-4">{rec.desc}</p>
              <div className="bg-white border border-rl-border rounded-xl p-3.5 mb-4">
                ...
                <div className="text-[14px] font-bold text-rl-forest mb-0.5">{rec.first}</div>
