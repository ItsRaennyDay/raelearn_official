...
function button(href: string, label: string): string {
  return `<a href="${href}" style="display:inline-block;background:#2A5230;color:#FFFFFF;text-decoration:none;font-weight:bold;font-size:14px;padding:12px 24px;border-radius:10px;margin-top:20px;">${label}</a>`;
}
...
export function welcomeEmail(name: string) {
  ...
  return {
    ...
    html: layout(`
      <h1 style="margin:0 0 16px;font-size:22px;color:#2A5230;">Welcome, ${firstName}!</h1>
      ...
      ${button(`${BASE_URL}/courses`, "Browse Courses →")}
    `),
  };
}

export function enrollmentEmail(name: string, courseTitle: string) {
  const firstName = name.split(" ")[0] || name;
  return {
    subject: `You're enrolled in ${courseTitle}`,
    html: layout(`
      <h1 style="margin:0 0 16px;font-size:22px;color:#2A5230;">You're in, ${firstName}!</h1>
...
export function certificateEmail(name: string, courseTitle: string, certificateId: string) {
  const firstName = name.split(" ")[0] || name;
  return {
    subject: `Your certificate for ${courseTitle} is ready`,
    html: layout(`
      <h1 style="margin:0 0 16px;font-size:22px;color:#2A5230;">Congratulations, ${firstName}!</h1>
      <p>Your certificate for completing <strong>${courseTitle}</strong> has been issued.</p>
      ${button(`${BASE_URL}/dashboard/certificates/${certificateId}`, "View Certificate →")}
    `),
  };
}
